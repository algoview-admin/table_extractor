...
def gen_step3_pivot_scale_test():
    # PIVOT_PERFORMANCE_WARNING_THRESHOLD（1000）を超える1001種類の属性を持つ
    # KVペア表。2つのキー（拠点）× 1001属性 = 2002行。normalize_tables()が
...
def gen_step3_column_collision_test():
    ...
    # シート3: 括弧書き注釈の分離 — 括弧内から分離される新規列名（LLM命名を
    # 想定してテスト側でスタブする）と、既存の「拠点種別」列が衝突する。
    ws3 = wb.create_sheet("括弧注釈衝突")
    rows3 = [
        ["拠点", "拠点種別", "売上"],
        ["東京（本社）", "直営", 1200],
        ["大阪（支社）", "代理店", 980],
    ...
    # シート4: 階層圧縮カラムの展開 — インデント階層の最上位レベル名（LLM命名を
    # 想定してテスト側でスタブする）と、既存の「地域」列が衝突する。
    ws4 = wb.create_sheet("階層圧縮衝突")
    rows4 = [
        ["組織階層", "地域", "売上"],
        ["東日本", "首都圏", 160],
        ["  東京支社", "首都圏", 90],
