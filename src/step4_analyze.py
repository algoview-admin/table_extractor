...
処理概要: 正規化済みテーブルリストを OpenAI API に送信し、テーブルの意味・関係を分析する。
...
def format_relation_facts(relations: List[Dict], tables: List[DetectedTable]) -> str:
    ...
    for r in relations:
        ...
        c_parts = [
            f"{c}" + (f"（{id_to_title[c]}）" if id_to_title.get(c) else "")
            for c in cids
        ]
        lines.append(
            f"  {p_label}  ≈  " + " + ".join(c_parts) + f"   [数値一致率 {pct}%]"
        )
...
def _match_ratio(
    parent: DetectedTable, children: List[DetectedTable], tol: float = 0.03
) -> float:
    """
    ...
    for i in range(2, n):
        ...
        if len(smaller) > MAX_SMALLER:
            same = sorted(
                ...
                key=lambda t: totals[t.table_id],
                reverse=True,
            )
            other = sorted(
                [t for t in smaller if t.sheet_name != parent.sheet_name],
...
def _drop_supersets(relations: List[Dict]) -> List[Dict]:
    ...
    for _, group_iter in groupby(sorted(relations, key=keyfn), key=keyfn):
        ...
        result.extend(r for s, r in sets if not any(other < s for other, _ in sets))
    return result
  ...
  "table_analyses": [
    {
      ...
      "display_name": "テーブルを一意に識別できる簡潔な表示名（意味の重複を避けること。詳細は下記【display_name生成ルール】参照）",
...
【display_name生成ルール（意味の重複回避・最重要）】
display_name は、そのテーブルが「どのシート/区分」の「何のデータ」かを人が一目で
理解できる、簡潔で重複のない名前にすること。

判定手順:
  1. まずテーブルの内容（タイトル行・列名・値）から、内容を的確に表す名前案を考える。
  2. その名前案に、シート名が伝える情報（区分・指標・期間等）が意味として
     既に含まれているかを確認する。
     - 語彙が完全一致でなくても、同じ役割の情報（例:「支店別」と「エリア別」は
       どちらも区分軸を示す語、シート名「業績サマリ」がタイトルに既に
       含まれる、等）を表していれば「含まれている」とみなす。
  3. 既に含まれていれば、シート名を重ねて追加しない。名前案をそのまま使う。
  4. 含まれていない情報（例: どの支店/どの期間か）があれば、その不足分だけを
     名前案に補う。シート名をまるごと機械的に連結してはいけない。

❌ 誤例: シート名「業績サマリ」＋内容「2024-2025 業績サマリ（四半期別）」
  → 誤った表示名:「業績サマリ 2024-2025 業績サマリ（四半期別）」
    （「業績サマリ」が意味として二重に含まれる）
  → 正しい表示名:「2024-2025 業績サマリ（四半期別）」
    （シート名の情報は既に内容に含まれているため追加しない）

❌ 誤例: シート名「支店別（プラン数）」＋内容「プラン数販売実績（エリア別）」
  → 誤った表示名:「支店別（プラン数） プラン数販売実績（エリア別）」
    （「プラン数」の重複、「支店別」と「エリア別」も同じ区分軸の重複）
  → 正しい表示名:「プラン数販売実績（エリア別）」

✅ 正例: シート名「横浜支店」＋内容「サービスA合計」（シートと内容が独立した情報）
  → 表示名:「横浜支店 サービスA合計」（重複がないためそのまま連結してよい）

【granularity_levelの定義】
- detail: 全ての集計軸において最下位レベルのデータ。これ以上分解されない最小単位
- summary: 1つ以上の集計軸で上位集計されているデータ（組織合計・サービス合計・時間合計など）
  ※「上位集計」とは、ファイル内の別テーブルを集計した結果であることを指す。
    単一テーブルの内部に「合計」「計」等の名称を持つ行・列や指標が含まれるだけでは summary としない
    （例: 1テーブル内に指標として「労働力人口」と、その内訳である「就業者数」「完全失業者数」が
    行として混在していても、これはテーブル間の親子関係ではないため summary の根拠にならない）。
...
▼ 多段階（ネスト）集計の識別（最重要）:
  ...
  ★ 優先度E'（孤立テーブル・単独ファイル）:
    ファイル内に検証対象となる他テーブルが存在しない、または「事前検証済み：数値合計関係」
    「シート間集計構造（推定）」「テーブル外注記」のいずれにもこのテーブルに関する
    テーブル間の親子関係の記載が一切ない場合
    → granularity_level=detail かつ is_minimum_granularity_candidate=true をデフォルトとする。
    ※ summary はあくまで「ファイル内の別テーブルを集計した結果である」ことが根拠。
       ファイル名・テーブル名・指標名に「集計」「合計」等の語が含まれる、または
       テーブル内部の指標同士が計算上の内訳関係にあるように見えるだけでは、
       このルールを覆して summary と判定してはならない。

  ★ 優先度F（フォールバック）:
    上記A〜E'のいずれにも明確に該当しない場合のみ、観点1・2・3を総合的に判断する。
    → unknown は最終手段。名称・構造・注記から少しでも推定できる場合は summary または detail を選択。
    ただし他テーブルとの関係性が一切示されない状況で summary を選ぶことはできない
    （優先度E'が必ず先に適用されるため）。
...
def _format_table_detail(
    ...
    if t.df is not None and not t.df.empty:
        ...
        for _, row in t.df.iterrows():
            row_head = " ".join(
                str(v) for v in list(row)[:3] if v is not None and str(v).strip()
            )
            if any(m in row_head for m in _TOTAL_MARKERS):
                total_labels.append(row_head[:20])
        if total_labels:
            total_line = (
                f"  計・合計行の見出し例: {', '.join(dict.fromkeys(total_labels))}\n"
            )
...
def _format_merge_candidates(
    ...
    for (ptitle, k), entries in by_type.items():
        ...
        example_titles = [
            id_to_title.get(cid, cid) for cid in branch_entries[0]["child_ids"]
        ]
...
def _compute_merge_recommendations(
    ...
    for i, ((ptitle, k), entries) in enumerate(by_type.items(), 1):
        ...
        sheet_axis_parent: Optional[str] = (
            agg_entries[0]["parent_id"] if agg_entries else None
        )
        ...
        try:
            ...
            if (
                ...
                if attempt < 3:
                    ...
                    continue
            raise
    raise RuntimeError(f"API呼び出しが最大リトライ回数を超えました: {last_exc}")
...
def _merge_raws(
    ...
    filtered_phase1 = [
        r
        for r in phase1_int_recs
        if not any(
            frozenset(r.get("table_ids", [])) <= pool_set for pool_set in superset_pool
        )
    ]
...
def analyze_tables(
    ...
    if n <= _SINGLE_CALL_LIMIT:
        return _analyze_single(
            client, model, tables, sheet_names, relation_facts, sheet_level_hints
        )
...
def _parse_response(
    ...
    for ta in raw.get("table_analyses", []):
        ...
        table_analyses.append(
            TableAnalysisResult(
                ...
                parent_table_ids=[
                    x for x in ta.get("parent_table_ids", []) if x in valid_ids
                ],
                child_table_ids=[
                    x for x in ta.get("child_table_ids", []) if x in valid_ids
                ],
                similar_table_ids=[
                    x for x in ta.get("similar_table_ids", []) if x in valid_ids
                ],
                is_minimum_granularity_candidate=ta.get(
                    "is_minimum_granularity_candidate", False
                ),
    ...
    seen_relations: set = (
        set()
    )  # 重複排除のため (parent_id, frozenset(child_ids)) を追跡する
    for ir in raw.get("integration_recommendations", []):
        ...
        compat_parent_col = (
            axis_parent_cols[0]
            if axis_parent_cols
            else (ir.get("parent_label_column") or None)
        )
    ...
    for mt in raw.get("master_tables", []):
        ...
        master_tables.append(
            MasterTableInfo(
                ...
                referenced_by=[
                    x for x in mt.get("referenced_by", []) if x in valid_ids
                ],
