...
def normalize_tables(
    ...
    各テーブルに対し、次の順序で処理する（各処理の出力が次の処理の入力になる）:
      ...
      7. 階層圧縮カラムの展開（決定論的検出＋LLM命名） — 階層カテゴリが
         ...
         フォールバックして展開自体は実行する。展開直後、
         find_hierarchy_rollup_rows で1段深い子行群の合計と数値が一致する
         行（ロールアップ行。親レベルの小計・合計を表す行）を検出し、
         本機能自身が除去する（10.の集計行除去とは別の、集計キーワードに
         頼れない階層構造に対して同じ目的を果たす、本機能専用の除去ロジック。
         除去した行は t.hier_rollup_removed_metadata に監査用メタデータ
         として保持する）
      8. グルーピング列の前方補完
