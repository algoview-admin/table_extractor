...
▼ 最小粒度の判定ルール（最重要）:
  ...
  判定方法:
    - 「事前検証済み：数値合計関係」で左辺（親）に登場しない → 最小粒度候補
    - 「シート間集計構造（推定）」の集計シート候補のテーブル → NOT最小粒度（summary扱い）
    - 集計元シートのテーブルかつサービス/商品軸でも最下位（子テーブルなし） → true
  重要: 組織軸の上位シートに同一構造の集計テーブルが存在する場合でも、
    そのシートの元となる詳細データ（支店等の明細）は is_minimum_granularity_candidate=true。
