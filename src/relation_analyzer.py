...
【重要】通常、2種類のマスタが必要になる:
  マスタ1（組織軸）: 支店 × 事業部（支店がどの事業部に属するかを示す）
  マスタ2（サービス軸）: サービス区分 × 上位サービス区分（詳細サービスが属する上位サービス合計を示す）
  両方のマスタを生成するために、2軸統合で両軸に親テーブルを設定すること。

例（支店 × サービス区分 の2軸統合の場合）:
  new_column_names: ["支店", "サービス区分"]
  axis_parent_table_ids: ["東京事業部_T2", "ネクストタイプA合計_T5"]
    → 軸0(支店): 東京事業部_T2（事業部シートにある同構造テーブル）が親
      → 生成マスタ1: 支店 × 事業部（支店→事業部の対応）
    → 軸1(サービス区分): ネクストタイプA合計_T5（同サービスの上位集計テーブル）が親
      → 生成マスタ2: サービス区分 × 上位サービス区分（詳細サービス→合計区分の対応）
  axis_parent_label_columns: ["事業部", "上位サービス区分"]

親テーブル選定のルール:
  - シート軸（支店など）の親: そのシート軸の上位組織を示すシート上の任意テーブル
    → マスタの親値にはそのシートのシート名を使用
  - タイトル軸（サービスなど）の親: セクションタイトルレベルの集計テーブル（合計行/タイプ合計等）
    → マスタの親値にはそのテーブルのセクションタイトルを使用
  - 多段階（例: C-1,C-2 → タイプC合計 → ネクスト合計）の場合、直近の上位テーブルを親に指定
...
▼ 2軸統合のパターン（推奨）:
  シート内でサービス区分軸での統合が提案されている場合:
  → クロスシート統合で支店軸を追加し「支店 × サービス区分」の2軸統合を提案
  → これにより全データが1つの統合テーブルにまとまる

▼ axis_parent_table_ids の設定（マスタ2種を生成するために必須）:
  - 支店軸の親 → 事業部シートにある同構造テーブルのID（支店×事業部マスタを生成）
  - サービス区分軸の親 → 上位サービス集計テーブルのID（サービス上下関係マスタを生成）
  両軸に親を設定することで「支店×事業部マスタ」と「サービス上下関係マスタ」が両方生成される。

▼ 統合の一貫性チェック:
  各支店の同一サービステーブルは全て同じ統合グループに含めること。
